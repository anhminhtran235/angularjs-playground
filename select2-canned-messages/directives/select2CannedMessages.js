angular.module('select2CannedMessagesDirective', []).directive('select2CannedMessages', function () {
    return {
        restrict: 'E',
        scope: {
        ...
        link: function (scope, elements, attrs) {
            ...
            scope.addNewMessage = () => {
                ...
                if (scope.displayedMessagesMaxLimit && scope.displayedMessagesMaxLimit <= currentlyDisplayedMessages.length) {
                ...
                if (messageExistsButUnselected) {
                    ...
                    const newSelect2Option = new Option(newMessage, newMessage, true, true);
            ...
            const verifyAttributes = () => {
                if (!currentlyDisplayedMessages) {
                    throw new Error('Must have currentlyDisplayedMessages. Pass in an empty list if there is none');
                }
                if (!templateMessages) {
                    throw new Error('Must have templateMessages. Pass in an empty list if there is none');
                }
            };
            ...
            const generateSelect2Data = (currentlyDisplayedMessages, allMessages) => {
            ...
            const listenToSelect2Events = (sl2, currentlyDisplayedMessages, allMessages) => {
                sl2.on('change', () => {
            ...
            const select2Data = generateSelect2Data(currentlyDisplayedMessages, allMessages);

            $(document).ready(function () {
                const sl2 = $('#' + scope.uniqueHtmlId);
                initSelect2(sl2, select2Data);
                listenToSelect2Events(sl2, currentlyDisplayedMessages, allMessages);
            });
        },
    };
});
