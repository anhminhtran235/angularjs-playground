...
app.post('/email/webhook', async (req, res) => {
    ...
    newEvents.forEach((emailEvent) => {
        if (eventsOfInterest.includes(emailEvent.event)) {
            ...
            EmailTracking.findOne({ xMessageId }).exec(async (err, emailTracking) => {
                if (err) {
                    console.log(err);
                    return res.send(err);
                }
                if (emailTracking) {
                    ...
                    const isEventDuplicate = emailTracking.events.findIndex((emailEvent) => emailEvent.sgEventId === newEmailEvent.sgEventId) !== -1;
                    if (!isEventDuplicate) {
                        await EmailTracking.updateOne({ _id: emailTracking._id }, { $push: { events: newEmailEvent } });
                    }
                }
            });
        }
...
const eventsOfInterest = [PROCESSED, DROPPED, DELIVERED, DEFERRED, BOUNCE, OPEN];
